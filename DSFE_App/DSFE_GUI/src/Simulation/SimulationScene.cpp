...
namespace gui {
    ...
    void SimulationScene::set_material(uint32_t idx, const glm::vec3& albedo, float metallic, float roughness, float ao) {
        if (idx >= _renderables.size()) { return; }
        _renderables[idx].albedo = glm::vec4(albedo, 1.0f);
        _renderables[idx].material = glm::vec4(metallic, roughness, ao, 0.0f);
    }
