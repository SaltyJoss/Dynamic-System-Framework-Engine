...
TEST("GLRK2 Method", GLRK2_HarmonicOscillator_EnergyPreservation)
{
	...
	for (int i = 0; i < 2500; ++i) { x = ode.GLRK2(x, t, dt, impHarmonicOsc); t += dt; }
	double Ef = 0.5 * (x(0) * x(0) + x(1) * x(1));
	double drift = std::abs(Ef - E0);
...
TEST("GLRK2 Method", GLRK2_ExponentialDecay)
{
	...
	for (int i = 0; i < 1000; ++i) { x = ode.GLRK2(x, t, dt, impExpDecay); t += dt; }
...
TEST("GLRK2 Method", GLRK2_Stability_LargeStep)
{
	...
	double prev = x(0);
	for (int i = 0; i < 20; ++i) {
		x = ode.GLRK2(x, t, dt, impExpDecay);
		t += dt;
		ASSERT_TRUE(x(0) < prev * (1.0 + 1e-12), "Not monotone");
		ASSERT_TRUE(x(0) < prev, "Not strictly decaying");
		prev = x(0);
...
TEST("GLRK2 Method", GLRK2_LinearDecay)
{
	VecX x(1); x << 1.0;
	double T = 0.5, dt = T / 500, t = 0.0;
	for (int i = 0; i < 500; ++i) { x = ode.GLRK2(x, t, dt, linearDecay); t += dt; }
...
TEST("GLRK2 Method", GLRK2_StiffDecay)
{
	...
	for (int i = 0; i < 500; ++i) { x = ode.GLRK2(x, t, dt, [](double, const VecX& x) { return -100.0 * x; }); t += dt; }
...
TEST("GLRK2 Method", GLRK2_NonlinearDecay)
{
	...
	for (int i = 0; i < 500; ++i) { x = ode.GLRK2(x, t, dt, [](double, const VecX& x) { return -x * x; }); t += dt; }
...
TEST("GLRK2 Method", GLRK2_StiffNonlinearDecay)
{
	VecX x(1); x << 1.0;
	double T = 0.5, dt = T / 500, t = 0.0;
	for (int i = 0; i < 500; ++i) { x = ode.GLRK2(x, t, dt, [](double, const VecX& x) { return -100.0 * x * x; }); t += dt; }
...
TEST("GLRK3 Method", GLRK3_HarmonicOscillator_EnergyPreservation)
{
	...
	double E0 = 0.5 * (x(0) * x(0) + x(1) * x(1));
	double T = 5.0, dt = T / 2500, t = 0.0;
	for (int i = 0; i < 2500; ++i) { x = ode.GLRK3(x, t, dt, impHarmonicOsc); t += dt; }
	double Ef = 0.5 * (x(0) * x(0) + x(1) * x(1));
	double drift = std::abs(Ef - E0);
...
TEST("GLRK3 Method", GLRK3_ExponentialDecay)
{
	VecX x(1); x << 1.0;
	double T = 1.0, dt = T / 1000, t = 0.0;
	for (int i = 0; i < 1000; ++i) { x = ode.GLRK3(x, t, dt, impExpDecay); t += dt; }
...
TEST("GLRK3 Method", GLRK3_Stability_LargeStep)
{
	...
	double prev = x(0);
	for (int i = 0; i < 20; ++i) {
		x = ode.GLRK3(x, t, dt, impExpDecay);
		t += dt;
		ASSERT_TRUE(x(0) < prev * (1.0 + 1e-12), "Not monotone");
		ASSERT_TRUE(x(0) < prev, "Not strictly decaying");
		prev = x(0);
...
TEST("GLRK3 Method", GLRK3_LinearDecay)
{
	VecX x(1); x << 1.0;
	double T = 0.5, dt = T / 500, t = 0.0;
	for (int i = 0; i < 500; ++i) { x = ode.GLRK3(x, t, dt, linearDecay); t += dt; }
...
TEST("GLRK3 Method", GLRK3_StiffDecay)
{
	...
	double T = 0.5, dt = T / 500, t = 0.0;
	double expected = std::exp(-50.0);
	for (int i = 0; i < 500; ++i) { x = ode.GLRK3(x, t, dt, [](double, const VecX& x) { return -100.0 * x; }); t += dt; }
...
TEST("GLRK3 Method", GLRK3_NonlinearDecay)
{
	...
	double T = 0.5, dt = T / 500, t = 0.0;
	double expected = 1.0 / (1.0 + T);
	for (int i = 0; i < 500; ++i) { x = ode.GLRK3(x, t, dt, [](double, const VecX& x) { return -x * x; }); t += dt; }
...
TEST("GLRK3 Method", GLRK3_StiffNonlinearDecay)
{
	...
	for (int i = 0; i < 500; ++i) { x = ode.GLRK3(x, t, dt, [](double, const VecX& x) { return -100.0 * x * x; }); t += dt; }
	double rel_err = std::abs(x(0) - expected) / expected;
