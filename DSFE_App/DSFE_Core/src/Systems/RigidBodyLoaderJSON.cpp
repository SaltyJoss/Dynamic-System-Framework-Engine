...
namespace systems {
	...
	// --- RigidBodyLoader Link and Joint Parsing ---

	// link parsing

	// Parse materials block if present
	// Each material is defined as: "materials": { "mat_name": [r, g, b, a] }
	static void loadMaterials(const json& data, RigidBodyModel& rigidBody) {
	...
	static void parseVisual(const json& linkData, const std::unordered_map<std::string, Vec4>& materials, RigidBodyLink& link) {
	...
	static void parseJointOrigin(const json& jointData, RigidBodyJoint& joint) {
	...
	static void parseJointAxis(const json& jointData, RigidBodyJoint& joint) {
	...
	static void parseJointLimits(const json& jointData, RigidBodyJoint& joint) {
	...
	static void parseJointDynamics(const json& jointData, RigidBodyJoint& joint) {
	...
	RigidBodyModel RigidBodyLoader::loadFromJSON(const std::string& filepath) {
		RigidBodyModel rigidBody;
		LOG_INFO("Loading rigidBody model from JSON: %s", filepath.c_str());
		D_INFO("Loading rigidBody model from JSON: %s", filepath.c_str());
		...
		rigidBody.name = data["name"].get<std::string>();
		loadMaterials(data, rigidBody);

		// Visual frame (optional, defaults to JOINT)
		if (data.contains("visual_frame")) {
			const std::string vf = data["visual_frame"].get<std::string>();
			if (vf == "joint")		{ rigidBody.visualFrame = eVisualFrame::JOINT; }
			else if (vf == "link")  { rigidBody.visualFrame = eVisualFrame::LINK; }
			else if (vf == "world") { rigidBody.visualFrame = eVisualFrame::WORLD; }
			else { LOG_WARN("Unknown visual_frame '%s', defaulting to JOINT", vf.c_str()); }
		}
		else {
			rigidBody.visualFrame = eVisualFrame::JOINT;
		}

		// Load rigidBody scale (default 1.0)
		rigidBody.scale = data.value("scale", 1.0f);

		// Load base frame if present
		if (data.contains("base_frame")) {
			rigidBody.baseFrameIsEngineAligned = false;
			...
			rigidBody.baseFrame = Mat4::Identity();
			rigidBody.baseFrame.block<3, 3>(0, 0) = q.toRotationMatrix();
			rigidBody.baseFrame.block<3, 1>(0, 3) = t;
			...
			rigidBody.baseFrameIsEngineAligned = true;
			rigidBody.baseFrame = Mat4::Identity();
		...
		rigidBody.kinematicsModel = decideKinematicsModel(data);
		...
		for (auto& jointData : data["joints"]) {
			...
			rigidBody.joints.push_back(joint);

			// If rigidBody is DH-mode, also parse DH table
			if (rigidBody.kinematicsModel == eKinematicsModel::DH) {
				DH_Params<double> dh{};
				if (!parseDHParameters(jointData, dh)) {
					LOG_WARN("Joint %s missing 'dh' unexpectedly; forcing URDF mode.", joint.name.c_str());
					rigidBody.kinematicsModel = eKinematicsModel::URDF;
					rigidBody.dhParams.clear();
				}
				else {
					rigidBody.dhParams.push_back(dh);
				}
			}
		...
		if (rigidBody.kinematicsModel == eKinematicsModel::URDF) { rigidBody.dhParams.clear(); }

		LOG_INFO("RigidBody loaded: %d links, %d joints", (int)rigidBody.links.size(), (int)rigidBody.joints.size());
		D_SUCCESS("RigidBody loaded: %d links, %d joints", (int)rigidBody.links.size(), (int)rigidBody.joints.size());
