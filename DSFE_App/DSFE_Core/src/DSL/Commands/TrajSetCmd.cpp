...
namespace commands {
	...
	CmdResult TrajSetCmd::update(CommandContext& cntx, double dt) {
		...
		if (body->hasFreeJoint()) {
			SIM_FAIL("trajSet: '%s' is a free body; trajectories apply only to articulated joints.", member.c_str());
		...
		if (typeU == "SINE" || typeU == "SIN") {
			...
			if (!body->trySetJointOmegaRefMaxRad(_link, wMax_est)) {
				D_WARN("trajSet(SINE): failed to set joint omega ref max for link='%s'", member.c_str());
			}

			SIM_SUCCESS("trajSet: SINE link='%s' dur=%.6fs centre=%.6f amp=%.6f f=%.6fHz phi=%.6f", member.c_str(), dur, centre, amp, fHz, phi);
		...
		if (typeU == "MSINE" || typeU == "MULTISINE") {
			...
			if (!body->trySetJointOmegaRefMaxRad(_link, wMax_est)) {
				D_WARN("trajSet(MSINE): failed to set joint omega ref max for link='%s'", member.c_str());
			}
			auto traj = std::make_unique<control::MultisineTrajectory<double>>(t0, t0 + dur, centre, std::move(comps));
			auto& trajMgr = core->trajectoryManager();
			trajMgr.set(member, std::move(traj));
			SIM_SUCCESS("trajSet: MSINE link='%s' dur=%.6fs centre=%.6f nComps=%zu", member.c_str(), centre, dur, nComps);
			_done = true;
			markCompleted();
