...
namespace commands {
	...
	// Constructor
	SpinCmd::SpinCmd(utils::AxisMask axes, double omegaDeg, double duration)
		: _axes(axes), _omegaDeg(omegaDeg), _remainingTime(duration), _started(false) {
	...
	std::unique_ptr<ICommand> CreateSpinCmd(const std::string& id, const std::vector<std::string>& args) {
		...
		return std::make_unique<SpinCmd>(axes, omegaOpt, durOpt);
	}
} // namespace commands