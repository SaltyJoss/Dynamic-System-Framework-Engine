...
namespace {
	...
	TEST("Division", DualNumber_Division_f) {
		...
		ASSERT_TRUE(std::abs(c.real - 0.5f) < 1e-5f, "Division real part incorrect");
		ASSERT_TRUE(std::abs(c.dual[0] - expected0) < 1e-5f, "Division dual[0] part incorrect");
	}
	...
	TEST("Division", DualNumber_DivisionIdentity) {
		...
		ASSERT_TRUE(std::abs(left.real - a.real) < 1e-12, "Division by identity real part incorrect");
		ASSERT_TRUE(std::abs(left.dual[0] - a.dual[0]) < 1e-12, "Division by identity dual[0] part incorrect");
		ASSERT_TRUE(std::abs(left.dual[1] - a.dual[1]) < 1e-12, "Division by identity dual[1] part incorrect");
	}
	...
	TEST("Division", DualNumber_DivisionByScalar) {
		...
		ASSERT_TRUE(std::abs(left.real - 0.5) < 1e-12, "Division by scalar real part incorrect");
		ASSERT_TRUE(std::abs(left.dual[0] - 0.25) < 1e-12, "Division by scalar dual[0] part incorrect");
		ASSERT_TRUE(std::abs(left.dual[1] - 0.125) < 1e-12, "Division by scalar dual[1] part incorrect");
	}
	...
	TEST("Division", DualNumber_DivisionByNegativeScalar) {
		...
		double scalar = -2.0;
		DualNumber_T<double, 2> left = a / scalar;
		ASSERT_TRUE(std::abs(left.real + 0.5) < 1e-12, "Division by negative scalar real part incorrect");
		ASSERT_TRUE(std::abs(left.dual[0] + 0.25) < 1e-12, "Division by negative scalar dual[0] part incorrect");
		ASSERT_TRUE(std::abs(left.dual[1] + 0.125) < 1e-12, "Division by negative scalar dual[1] part incorrect");
	}
	// Test that division by zero throws an exception for dual numbers
