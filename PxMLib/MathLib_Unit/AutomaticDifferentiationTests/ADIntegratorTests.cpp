...
namespace {
	...
	VecX_T<DualNumber_T<Scalar, NVar>> expDecay(DualNumber_T<Scalar, NVar>, const VecX_T<DualNumber_T<Scalar, NVar>>& x) {
	...
	template<typename Scalar, size_t NVar>
	VecX_T<DualNumber_T<Scalar, NVar>> harmonicOsc(DualNumber_T<Scalar, NVar>, const VecX_T<DualNumber_T<Scalar, NVar>>& s) {
	...
	VecX_T<DualNumber_T<Scalar, NVar>> integrateToTime(StepFn stepFn, const VecX_T<DualNumber_T<Scalar, NVar>>& x0, Scalar T, int N) {
	...
	void verifyExpDecaySensitivity(StepFn stepFn, double tol, int N) {
	...
	template<typename StepFn>
	void verifyHarmonicOscillatorSensitivity(StepFn stepFn, double tol, int N) {
		...
		VecX_T<DualNumber_T<double, 2>> r = integrateToTime(stepFn, x0, 2.0 * M_PI, N);
		double energy = 0.5 * (r(0).real * r(0).real + r(1).real * r(1).real);

		ASSERT_TRUE(std::abs(r(0).real - 1.0) < tol, "Harmonic oscillator position error too large");
		ASSERT_TRUE(std::abs(r(1).real - 0.0) < tol, "Harmonic oscillator velocity error too large");
		ASSERT_TRUE(std::abs(r(0).dual[0] - 1.0) < tol, "Harmonic oscillator position sensitivity error too large");
		ASSERT_TRUE(std::abs(r(1).dual[1] - 1.0) < tol, "Harmonic oscillator velocity sensitivity error too large");
		ASSERT_TRUE(std::abs(r(0).dual[1]) < tol, "Harmonic oscillator position sensitivity to velocity should be near zero");
		ASSERT_TRUE(std::abs(r(1).dual[0]) < tol, "Harmonic oscillator velocity sensitivity to position should be near zero");
		ASSERT_TRUE(std::abs(energy - 0.5) < tol, "Harmonic oscillator energy error too large");
	}
	...
	TEST("AD Euler Method", Euler_HarmonicOscillatorSensitivity) {
		...
		verifyHarmonicOscillatorSensitivity(stepFn, 5e-2, 2000);
	}
	...
	TEST("AD Midpoint Method (RK2)", Midpoint_HarmonicOscillatorSensitivity) {
		...
		verifyHarmonicOscillatorSensitivity(stepFn, 1e-5, 1000);
	}
	...
	TEST("AD RK4 Method", RK4_HarmonicOscillatorSensitivity) {
		...
		verifyHarmonicOscillatorSensitivity(stepFn, 1e-5, 100);
	}
